...
- [ ] `cd source && bun run lint` passes with no errors
- [ ] `cd source && bun run format` passes with no errors
- [ ] `cd source && bun test` runs and all tests pass
- [ ] Tests cover: MCP initialization, `send` tool, `broadcast` tool, `list_agents` tool, inbox polling + channel delivery, message lifecycle (inbox -> processed)
- [ ] `.github/workflows/ci.yml` exists and runs lint + test on push and PR
- [ ] `CLAUDE.md` documents the lint and test commands
- [ ] `source/intercom.ts` passes lint with no changes to behavior
- [ ] `specs/protocol.md` is still accurate after changes (verify, no update expected)
