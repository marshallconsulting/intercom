...
> **Requires CDD v1.** This skill expects the repo to use Context-Driven Development folder structure (`specs/`). See `CDD.md` at the repo root.
...
| What to find | Where to look (Elixir/Phoenix) | Where to look (Rails) | Where to look (generic) |
|---|---|---|---|
| Business contexts | `lib/<app>/` subdirectories + context modules | `app/models/`, `app/services/` | Domain modules, service layers |
| Data model | Schema/model files, migrations | `db/schema.rb`, model files | ORM models, DB schemas |
| User-facing features | LiveViews/controllers, router | Controllers, routes | Routes, views, handlers |
| External integrations | Top-level lib modules (API clients, pollers) | `app/services/`, `lib/` | Client modules, API wrappers |
| Access control | Auth modules, role checks, feature flags | Pundit/CanCanCan policies | Auth middleware, guards |
| Background processing | Job modules, pollers, workers | Sidekiq/ActiveJob workers | Queue consumers, cron |

**For each domain area, capture:**
- What it does (business purpose, not implementation)
- Key entities and their relationships
- Important business rules ("credits expire FIFO", "odds must be >= +100 or <= -100")
...
- One business domain (accounts, billing, events)
- One major subsystem (prediction engine, trading system)
- One cross-cutting concept (the betting domain glossary, data pipeline architecture)
...
| `accounts.md` | Users, roles, preferences |
| `billing.md` | Subscriptions, tiers, feature gating |
| ... | ... |

**Already covered:** betting-domain.md (existing, will not overwrite)
...
Reference other specs by name: "See [Billing](billing.md) for subscription tiers."
```

**Writing guidelines:**

- **Lead with what, then why.** "Credits expire after 12 months. This prevents balance hoarding and simplifies accounting."
- **Use concrete examples.** "Lakers -150 vs Celtics +130" is better than "negative odds indicate favorites."
- **Include business rules as declarative statements.** "American odds must always be >= +100 or <= -100, never in the range -99 to +99."
...
- **Name the entities.** An agent reading the spec should know the vocabulary: "A Pick is a system-generated recommendation. A Bet is a user-placed wager."
- **Don't describe the UI.** Specs say what the system *is*, not what the screens look like.
- **Don't prescribe implementation.** No file paths, no function signatures, no "this should be a GenServer."
...
- **Specs are domain knowledge, not documentation.** They explain what the system *is* and why, not how to use or build it. "Credits expire FIFO" is a spec. "Run `mix test` to run tests" is not.
